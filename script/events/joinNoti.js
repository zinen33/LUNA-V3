module.exports.config = {
    name: "joinNoti",
    eventType: ["log:subscribe", "log:unsubscribe"], // Added "log:unsubscribe"
    version: "1.0.1",
...
module.exports.run = async function ({ api, event }) {
    ...
    if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
        return api.sendMessage("إفسحو المجال قد أتت الملكة 😎", event.threadID);
    }

    // Handling developer leaving the group
    if (event.logMessageData.leftParticipantFbId) {
        const leftUserID = event.logMessageData.leftParticipantFbId;
        if (developerIDs.includes(leftUserID)) {
            return api.sendMessage("غادر زينو 😔", event.threadID);
        }
    }
